...
def rotate_data(data_dir, augment_both=True):
    ...
    degrees = [90, 180, 270] #[19, 23*2,19*3,90,19*6,23*6,23*7,180,23*9,23*10,19*13,270,19*15,23*14,19*18]
...
def augment_data(base_dir,augment_both, use_all_type=True):
    ...
    :return:
    """
    print('=========== Data augmentation just for 512 x 512 image size ==============')
    if augment_both:
        augment_gt=None
    else:
    ...
    splitting_up = use_all_type #False #use_all_type #use_all_type True to augment by splitting up
    rotation = use_all_type
    flipping = use_all_type
    correction_gamma = use_all_type

    #Fine Tuning Dataset Version 1\\Fine Tuning Frames Collated
    data_dir = os.path.join(base_dir, 'edges') #'BIPED','edges')
