...
def canny_edge_extraction(image_frame_paths, gt_dir_path):#, equalised_img_path):
    print(image_frame_paths)
    for frame in image_frame_paths[:-1]:
        ...
        #equalised_image = cv2.equalizeHist(image_grayscale)
        #cv2.imshow('equalised grayscale', equalised_image)
        ...
        #cv2.imwrite(os.path.join(equalised_img_path, head_tail[1]), adjusted)
...
def main(args):
    ...
    for subdir in os.listdir(endomapper_dir)[:-1]:
        ...
        #rescaled_grayscale_path = os.path.join(endomapper_dir, subdir, 'adjusted_depth_map')
        
        if not os.path.exists(gt_dir_path):
            os.mkdir(gt_dir_path)
        #if not os.path.exists(rescaled_grayscale_path):
        #    os.mkdir(rescaled_grayscale_path)

        image_frame_paths_list = image_frame_paths(depth_dir_path)
        canny_edge_extraction(image_frame_paths_list, gt_dir_path)#, rescaled_grayscale_path)
